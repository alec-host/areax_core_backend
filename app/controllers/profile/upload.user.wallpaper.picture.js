...
module.exports.UploadWallpaperPicture = async(req,res) => {
    ...
    try{
       ...
       const imageName = 'wallpaper'+'-'+email.split('@')[0];
       ...
       awsBucket(imageName,fileType,imagePath).then(async fileUrl => {
           const payload = { wallpaper_picture_url: fileUrl };
           ...
           res.status(200).json({
               ...
               error: false,
               file_url: fileUrl,
               message: "Wallpaper has been uploaded."
           });
       }).catch(err => {
           res.status(400).json({
               success: false,
