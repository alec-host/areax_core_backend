...
const walletService = (DATABASE_DIALECT && DATABASE_DIALECT !== 'mongo')
    ? require("../user/accounts/get.pg.wallet.transactions")
    : require("../user/accounts/get.mongo.wallet.transactions");
...
module.exports.GetWalletTransactions = async (req, res) => {
    ...
    try {
        ...
        const result = await walletService.getUserWalletTransaction(page, limit, reference_number);
        if (result) {
            res.status(200).json({
                success: true,
                error: false,
